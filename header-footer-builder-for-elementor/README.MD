...
**Header Footer Builder for Elementor** — create headers, footers & a mega menu for WordPress & WooCommerce. Beginner-friendly & optimized.
...
- **Sticky Header**: Make your Elementor sticky header remain at the top of the page when scrolling.
...
= **Mega Menu** =
- **Assign Elementor Templates**: Turn any top-level menu item into an Elementor mega menu by assigning an Elementor template to it — right from Appearance → Menus.
...
- **Completely Free**: Unlock all advanced features completely free
...
- Select the page if the footer is for a specific page, or leave it if it applies globally.  
- Set **display conditions** for pages or post types.  
- Select pages to **Exclude** the footer from if needed, or leave it blank to apply globally.  
- Click the **Create** button; you will be redirected to the Elementor editor
...
= Q: Can I create an Elementor sticky header using this plugin? =
...
= Version 1.3.3 =
 ...
 - Fixed : Fatal errors and header/footer showing without CSS (FOUC) issues.
 - Improved : Mega Menu dropdown, mobile menu, and per-item settings.

= Version 1.3.2 =

 - Fixed : Recommendation notice no longer reappears on every login after dismissal — dismissal is now saved per user.
 - Improved : Recommendation notice redesigned to be compact and responsive on all screen sizes.
...
= Version 1.2.0 = 
 - New Features : Added Plugin Dashboard to make the header and footer easier
